...
```
이와 같이 나는 들었습니다.          ← 표시 없음 = 평문(서술)

> 세존이시여 참으로 놀랍습니다. <    ← 다른 사람의 말

>> 아난다여 그렇게 말하지 말라. <<   ← 부처님 말씀
```

→ 대화는 옅은 판에, 부처님 말씀은 금빛 판에 담겨 한눈에 구분된다
→ **여는 표시부터 닫는 표시까지가 한 판**이다. 그 안에 문단이 여럿이면 판 하나로 이어진다
...
이 법을 깨닫지 못하기 때문에 중생들은 엉킨다. <<
```
→ 세 문단이 판 하나에 담긴다. 문단마다 표시를 붙이지 않는다

→ `세존께서 말씀하셨습니다` 처럼 말을 이끄는 서술은 감싸지 않는다 (평문)
→ 닫는 표시를 빠뜨리면 글 끝에서 저절로 닫힌다 (판이 이상하게 길어지면 그 신호다)
→ `>` `<` 는 본문에 쓰이지 않는 글자라 다른 것과 부딪히지 않는다
→ 표시가 없는 글은 예전 그대로 보인다 - 기존 명언은 손대지 않아도 된다
→ 중첩(부처님이 남의 말을 인용)은 아직 지원하지 않는다. 필요해지면 그때 더한다
→ 판 색은 `lib/theme.ts` 의 `talkBg`·`sayBg` 등에서 바꾼다
...
2. **루비/굵게/링크 렌더링**: RubyText 컴포넌트 (루비 클릭 → 주석 팝업 + 주석 추가/수정)
   - 평문 · 대화(`> <`) · 부처님 말씀(`>> <<`) 을 나눠 판에 담는다 (`splitSpeechBlocks`)
...
설정 > 번역 프롬프트의 기본값(`components/PromptModal.tsx`)에 화자 구분 규칙이
들어 있다. 프롬프트를 손봐 저장해 두었다면 그 값이 localStorage 에 남아 있으므로,
새 규칙을 쓰려면 그 화면에서 **초기화**를 한 번 눌러야 한다.
